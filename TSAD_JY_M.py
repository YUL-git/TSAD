...
parser = ArgumentParser(description="Multi-variate TSAD")
...
## unique_id 로 구분하여 다변량 시계을 만드는 구조 (특이함)
...
train_data = data.melt(id_vars=['ds'], value_vars=unique_id_cols, 
                        var_name='unique_id', value_name='y')

test1['ds'] = pd.to_datetime(test1.index, unit='s')
test_data1  = test1.melt(id_vars=['ds'], value_vars=unique_id_cols,
                        var_name='unique_id', value_name='y')

test2['ds'] = pd.to_datetime(test2.index, unit='s')
test_data2  = test2.melt(id_vars=['ds'], value_vars=unique_id_cols,
                        var_name='unique_id', value_name='y')

test3['ds'] = pd.to_datetime(test3.index, unit='s')
test_data3  = test3.melt(id_vars=['ds'], value_vars=unique_id_cols,
                        var_name='unique_id', value_name='y')

test4['ds'] = pd.to_datetime(test4.index, unit='s')
test_data4  = test4.melt(id_vars=['ds'], value_vars=unique_id_cols,
                        var_name='unique_id', value_name='y')

from neuralforecast import NeuralForecast
from neuralforecast.models import TSMixer, NHITS, PatchTST, TimesNet
...
model_name_list = ['TimesNet', 'PatchTST', 'TSMixer', 'NHITS']
